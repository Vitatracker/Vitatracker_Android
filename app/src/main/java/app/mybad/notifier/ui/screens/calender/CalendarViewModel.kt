...
class CalendarViewModel @Inject constructor(
    ...
    init {
        scope.launch {
            ...
            scope.launch {
                loadCourses.getCoursesFlow(userId).collect { _state.emit(_state.value.copy(courses = it)) }
            }
            scope.launch {
                loadCourses.getMedsFlow(userId).collect { _state.emit(_state.value.copy(meds = it)) }
            }
            scope.launch {
                loadCourses.getUsagesFlow(userId).collect { _state.emit(_state.value.copy(usages = it)) }
            }
        }
    }
