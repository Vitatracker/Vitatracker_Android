...
fun LocalDateTime.toDateFullDisplay(): String = this.toInstant(TimeZone.UTC).formatDateFull()
...
fun LocalTime.changeTime(hour: Int, minute: Int) = LocalTime(
    hour = hour,
    minute = minute,
    second = 0,
    nanosecond = 0,
...
fun LocalDateTime.changeTime(time: LocalTime) = LocalDateTime(
    year = this.year,
    monthNumber = this.monthNumber,
    dayOfMonth = this.dayOfMonth,
    hour = time.hour,
...
fun LocalDateTime.atStartOfDayInEpochSeconds() = this.atStartOfDay().toEpochSecond()

fun Long.atStartOfDay() = Instant.fromEpochSeconds(this)
    .toLocalDateTime(TimeZone.UTC)
    .atStartOfDayInEpochSeconds()
...
fun LocalDateTime.atEndOfDayInEpochSeconds() = this.atEndOfDay().toEpochSecond()

fun Long.atEndOfDay() = Instant.fromEpochSeconds(this)
    .toLocalDateTime(TimeZone.UTC)
    .atEndOfDayInEpochSeconds()
...
fun getCurrentTime() = getCurrentDateTime().let { LocalTime(hour = it.hour, minute = it.minute) }
