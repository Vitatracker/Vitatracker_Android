...
import app.mybad.domain.usecases.notification.SetNotificationPatternUsageUseCase
import kotlinx.datetime.LocalDateTime
import javax.inject.Inject

class SetFactUseTimeOrInsertUsageUseCase @Inject constructor(
    private val repository: UsageRepository,
    private val setNotificationPatternUsageUseCase: SetNotificationPatternUsageUseCase,
) {
    ...
    suspend operator fun invoke(
        ...
        // проверим оповещение
        setNotificationPatternUsageUseCase(
            userId = usageDisplay.userId,
            courseId = usageDisplay.courseId,
            patternId = if (usageDisplay.isPattern) usageDisplay.id else null,
            timeInMinutes = usageDisplay.timeInMinutes,
            isNotification = factUseTime != null,
        )
